...
np.save(out_dir+"lcos_chain.npy",chain)

post = chain[burn:].reshape(-1,3)
pd.DataFrame(post,columns=["s0","H0rd","MB"]).to_csv(out_dir+"lcos_post.csv",index=False)
...
plt.savefig(out_dir+"lcos_corner.png")
